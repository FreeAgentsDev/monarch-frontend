...
function deepCloneEstado(data: { countries: CountryData[] }) {
...
export default function EstadoResultadosView({ data }: EstadoResultadosViewProps) {
  ...
  const handleCellChange = useCallback((countryId: string, conceptIdx: number, mes: typeof MESES[number], value: number | '') => {
    setEditableData((prev: { countries: CountryData[] }) => {
      const next = deepCloneEstado(prev)
      const country = next.countries.find((c: CountryData) => c.id === countryId)
      if (!country || !country.concepts[conceptIdx]) return prev
      const row = country.concepts[conceptIdx]
      const num = value === '' ? 0 : Number(value)
      ;(row as Record<string, number>)[mes] = isNaN(num) ? 0 : num
  ...
  const sections = useMemo(() => {
    ...
    concepts.forEach((row: ConceptRow) => {
  ...
  const consolidatedData = useMemo(() => {
    if (!editableData?.countries?.length) return []
    ...
    editableData.countries.forEach((c: CountryData) => {
      c.concepts.forEach((row: ConceptRow) => {
        const key = row.concept
        if (!conceptMap.has(key)) {
          conceptMap.set(key, { concept: key, section: row.section, isMargin: isConceptMargin(key), byCountry: {}, total: 0 })
        }
        ...
        if (!entry.isMargin) entry.total += inCOP
          ...
          const vals = Object.values(entry.byCountry).filter((x: number) => x !== 0)
          entry.total = vals.length ? vals.reduce((a: number, b: number) => a + b, 0) / vals.length : rowTotal
        }
      })
    })
    const bySection = new Map<string, { concept: string; section?: string; isMargin: boolean; byCountry: Record<string, number>; total: number }[]>()
    conceptMap.forEach((v) => {
      const s = v.section ?? 'otros'
      if (!bySection.has(s)) bySection.set(s, [])
      bySection.get(s)!.push(v)
    })
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex rounded-lg border border-gray-200 p-1 bg-gray-50">
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md ${viewMode === 'por-pais' ? 'bg-white shadow-sm text-primary-700' : 'text-gray-600'}`}
          >
            Por país
          </button>
          <button
            ...
            className={`px-3 py-1.5 text-sm font-medium rounded-md flex items-center gap-1.5 ${viewMode === 'consolidado' ? 'bg-white shadow-sm text-primary-700' : 'text-gray-600'}`}
          >
            <Globe size={14} />
            Consolidado COP
          </button>
        ...
        >
          {editableData.countries.map((c: CountryData) => (
      ...
      {viewMode === 'consolidado' ? (
        <div className="card overflow-hidden p-0">
          <div className="px-4 py-3 bg-primary-50/50 border-b border-gray-200">
            <p className="text-sm text-gray-700">Valores convertidos a COP según las tasas configuradas.</p>
          </div>
            ...
            <table className="w-full min-w-[800px] text-sm">
              <thead>
                <tr className="bg-gray-100 border-b border-gray-200">
                  ...
                  {editableData.countries.map((c: CountryData) => (
                    <th key={c.id} className="text-right py-3 px-3 font-semibold text-gray-700">{c.name} (COP)</th>
                  ))}
              ...
              <tbody>
                {consolidatedData.map((row) => (
                  <tr key={row.concept} className={`border-b border-gray-100 ${row.concept.includes('UTILIDAD OPERACIONAL') ? 'bg-primary-50/50 font-medium' : ''}`}>
                    <td className="py-2 px-4 pl-8 font-medium text-gray-900">{row.concept}</td>
                    {editableData.countries.map((c: CountryData) => (
                      <td key={c.id} className="text-right py-2 px-3 tabular-nums text-gray-700">{formatValue(row.byCountry[c.id], row.isMargin)}</td>
                    ))}
                    <td className="text-right py-2 px-4 tabular-nums font-medium text-gray-900 bg-primary-50/30">{formatValue(row.total, row.isMargin)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
