...
// This keeps us under Vercel's Hobby plan limit of 12 functions
...
module.exports = async function handler(req, res) {
  ...
  try {
    // Get path from URL or query params (Vercel catch-all routes)
    // Try multiple methods to get the path
    let path = '';
    
    // Method 1: From req.query.path (catch-all route parameter)
    if (req.query && req.query.path) {
      const pathArray = req.query.path;
      path = Array.isArray(pathArray) ? pathArray.join('/') : (pathArray || '');
    }
    
    // Method 2: From req.url (direct URL access)
    if (!path && req.url) {
      const urlPath = req.url.split('?')[0]; // Remove query string
      // Remove /api prefix if present
      path = urlPath.replace(/^\/api\//, '').replace(/^\//, '');
    }
    
    // Method 3: From query string directly
    if (!path && req.query) {
      // Try to reconstruct from query params
      const segments = [];
      for (let i = 0; i < 10; i++) {
        if (req.query[`path[${i}]`]) {
          segments.push(req.query[`path[${i}]`]);
        }
      }
    ...
    // Clean query params - remove path-related params
    const cleanQuery = { ...req.query };
    delete cleanQuery.path;
    // Remove path[0], path[1], etc. if present
    ...
    console.log('API Request:', { path, method, url: req.url, queryKeys: Object.keys(query) });
    ...
    console.error('Route not found:', { path, url: req.url, query: req.query });
    return sendError(res, 404, `Endpoint not found: ${path || 'empty path'}`);
  } catch (error) {
...
async function handleOrders(req, res, query, body, method) {
  try {
    ...
    return sendError(res, 500, `Error loading orders: ${error.message}`);
  }
}
...
async function handleOrderById(req, res, id, query, body, method) {
  try {
    ...
    console.error('Error in handleOrderById:', error);
    return sendError(res, 500, `Error loading order: ${error.message}`);
  }
}
...
async function handleDashboardStats(req, res) {
  try {
    ...
    console.error('Error in handleDashboardStats:', error);
    return sendError(res, 500, `Error loading dashboard stats: ${error.message}`);
  }
}
...
async function handleTransactions(req, res, query) {
  try {
    ...
    return sendError(res, 500, `Error loading transactions: ${error.message}`);
  }
}
...
async function handleBalanceReport(req, res, query) {
  try {
    ...
    console.error('Error in handleBalanceReport:', error);
    return sendError(res, 500, `Error loading balance report: ${error.message}`);
  }
}
...
async function handleIncomeReport(req, res, query) {
  try {
    ...
    console.error('Error in handleIncomeReport:', error);
    return sendError(res, 500, `Error loading income report: ${error.message}`);
  }
}
...
async function handleShops(req, res, query, body, method) {
  try {
    ...
    console.error('Error in handleShops:', error);
    return sendError(res, 500, `Error loading shops: ${error.message}`);
  }
}
...
async function handleShopById(req, res, id, query, body, method) {
  try {
    ...
    console.error('Error in handleShopById:', error);
    return sendError(res, 500, `Error loading shop: ${error.message}`);
  }
}
...
async function handleShopSync(req, res, id, body) {
  try {
    ...
    console.error('Error in handleShopSync:', error);
    return sendError(res, 500, `Error syncing shop: ${error.message}`);
  }
}
...
async function handleSyncLogs(req, res, query) {
  try {
    ...
    console.error('Error in handleSyncLogs:', error);
    return sendError(res, 500, `Error loading sync logs: ${error.message}`);
  }
}
